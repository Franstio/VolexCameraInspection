...
namespace VolexCameraInspection.Services
{
    ...
    public class FolderPickingService : IFolderPickingService
    {
        ...
        public async Task<Uri?> PickFolder(Window? owner = null)
        {
            ...
            if (owner is null)
                return null;
            var pickDialog = await owner.StorageProvider.OpenFolderPickerAsync(new Avalonia.Platform.Storage.FolderPickerOpenOptions() { AllowMultiple = false, Title = "Pick FTP Folder" });
            return pickDialog.Count > 0 ? pickDialog[0].Path : null;
        }
